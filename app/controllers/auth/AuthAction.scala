...
final class AuthAction(auth: AuthenticationModule, redirect: Boolean, override val parser: BodyParser[AnyContent])(implicit ec: ExecutionContext)
  ...
  def invokeBlock[A](request: Request[A], block: (AuthRequest[A]) => Future[Result]) = {
    if (auth.isEnabled) {
      request.session.get(AuthAction.SESSION_USER).map { username => block(new AuthRequest(Some(User(username)), request)) }.getOrElse {
        if (redirect) {
          Future.successful(Results.Redirect(routes.AuthController.index).withSession(AuthAction.REDIRECT_URL -> request.uri))
        } else {
