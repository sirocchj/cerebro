...
class TemplatesController @Inject() (val authentication: AuthenticationModule, val hosts: Hosts, client: ElasticClient) extends BaseController {
  ...
  def delete = process { request =>
    ...
    client.deleteTemplate(name, request.target).map { response => CerebroResponse(response.status, response.body) }
  }

  def create = process { request =>
    val name     = request.get("name")
    val template = request.getObj("template")
    client.createTemplate(name, template, request.target).map { response => CerebroResponse(response.status, response.body) }
  }
