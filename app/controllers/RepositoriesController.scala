...
class RepositoriesController @Inject() (val authentication: AuthenticationModule, val hosts: Hosts, client: ElasticClient) extends BaseController {
  ...
  def save = process { request =>
    ...
    client.createRepository(name, repoType, settings, request.target).map { response => CerebroResponse(response.status, response.body) }
  }

  def delete = process { request =>
    val name = request.get("name")
    client.deleteRepository(name, request.target).map { response => CerebroResponse(response.status, response.body) }
  }
