...
class SnapshotsController @Inject() (val authentication: AuthenticationModule, val hosts: Hosts, client: ElasticClient) extends BaseController {
  ...
  def delete = process { request =>
    ...
    client.deleteSnapshot(repository, snapshot, request.target).map { response => CerebroResponse(response.status, response.body) }
  }
  ...
  def create = process { request =>
    ...
    client.createSnapshot(repository, snapshot, ignoreUnavailable, includeGlobalState, indices, request.target).map { response =>
  ...
  def restore = process { request =>
    ...
    client
      ...
      .map { response => CerebroResponse(response.status, response.body) }
  }
}
